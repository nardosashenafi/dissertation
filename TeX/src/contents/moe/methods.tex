...
dynamical systems. 
%
The objective of this framework is to learn a mixture of expert controller and
the responsibilities of the experts as determined by the gating network.
%
...
in depth in the next section~\todo{provide ref}.
%
...
\begin{enumerate}
    ...
    \end{equation}
    \noindent where $\mathcal{Q} \succ 0$~\todo{this symbol $\succ$ means p.d.
    matrix. Introduce and use it.} is positive definite matrix and $\mathcal{R} \succeq 0$ is
    a positive semi-definite matrix.
    %
    This construction encourages trajectories to reach the desired equilibrium
    with minimum effort and shortest time.~\todo[inline]{there is a tradeoff here.}
    %
    ...
    \end{align}
    \todo[inline]{explain algorithm, motivate how
    eqn.~\eqref{eq;accumulated_likelihood} is obtained.}
    %
    ...
    systems.~\todo{I didn't understand this sentence -- what is a trivial
    choice?}
    %
...
%
We leverage forward-mode auto-differentiation~\todo{discuss why not others?} technique~\cite{revels2016forward}
