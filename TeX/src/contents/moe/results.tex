...
unstable closed-loop systems to result in a piecewise-stable system.~\todo{cite
Liberzon}
%
...
%
The response of the resulting switching system is shown in
...
\todo[inline]{there is too much white space here!}
...
contact events~\todo{potential contacts} from two barriers as shown Figure~\ref{fig:cartpole_contact}. 
%
...
%
The pendulum spans length of $l=0.2$m~\todo{maybe use $\ell$ rather than $l$} and its mass $m_c = 0.75$kg is
...
% 
We constrain the maximum number of state partitions to 3.~\todo{would these be
...
% 
The cart uses a rack and pinion~\todo{cite Quanser} mechanism to translate on the track with zero-slip.
%
...
%
But~\todo{Try not to start sentences with but} from several trajectories, we have observed that the system leverages the
...
%
The MOE controllers~\todo{are there more than one MoE controllers? Isn't it just
\textit{one} MoE controller, which has many experts?} achieve successful swing-up in simulation and real-world,
...
the state partitions.~\todo{this can be explained better.}
%
...
%
But the controller is not able to catch the pendulum post-impact.~\todo{explain
