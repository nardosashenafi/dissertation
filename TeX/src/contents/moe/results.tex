...
\begin{align*}
    \mathbb{L}(\phi) = \int_{0}^{T} \Biggl( \sum_{i=1}^{N_F} - \Bigl[ \theta_i \ell \Bigl(x(t), u(\theta_i) \Bigr) + (1- \theta_i)\ell \Bigl(x(t), u(1-\theta_i) \Bigr) \Bigr] P\Bigl(c_i = i | x(t), \psi \Bigr)  \Biggr) \dd t.
\end{align*}
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}
        ...
        \draw[-stealth, very thick,black] (0.0,1.5) -- ++(-3.6, 0.0) node[above right,black]{\small $d$};
