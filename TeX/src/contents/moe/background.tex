...
\begin{algorithm}[H]
  \setstretch{1.2}
    ...
    \label{algo:moreau}
    \small
    \hspace*{\algorithmicindent} \textbf{Input}: $z(0) = (q(0), \dot{q}(0))$
    \begin{algorithmic}[1]
      \State $\phi \leftarrow  [z(0)]$ \Comment{Initial States}
        % \algrenewcommand\algorithmicindent{0em} % No indent
          \For{$t \in 0:\Delta t:T$} 
            ...
            \State $\phi \leftarrow \phi \cup [z(t+\Delta t)]$\Comment{Save trajectory}
          \EndFor
...
The mixture of experts (MOE) architecture is primarily used to fit high variance
or multi-modal datasets onto numerous models.
%
Given $N$ number of experts, a gating network divides the input space $z
\in \mathcal{Z}$ into regions (bins) within which a single expert is active.
%
The objective is to learn the expert models and the gating network that best fit
the dataset.
%
...
%
Let $f(z)$ denote a collection of expert models, i.e. $f(z) = \{f_k(z) :
\mathcal{Z} \rightarrow \mathcal{U} | \; 1 \leq k \leq N\}$.
%
The parameters of the experts are also given by a set $\theta=\{\theta_k | \;
1 \leq k \leq N\}$.
%
The output of the gating network $P(z) : \mathcal{Z} \rightarrow \mathbb{R}^N$
is the probability of the input $z$ belonging to each bin.
%
We use a categorical probability distribution to sample a bin based on the
outputs of the gating network as follows~\cite{harkonen2022mixtures} 
\begin{align}
  c_i | z, \psi \sim \text{Categorical}(P(z; \psi)) 
  \label{eq:gating_categorical}
\end{align}
\noindent where $c_i$ is the bin corresponding to the $k^{th}$ expert and $\psi$
is the parameters of the gating network.
%

There are several Bayesian inference techniques to train the expert models and
the gating network, one of which is \it{expectation maximization} \normalfont
(EM).
%
In this technique, we compose a likelihood function   
\begin{equation}
  \mathcal{L}(\norm{f(z)-y}{}) = \sum_{k=1}^{N} \norm{f_k(z; \theta_k)-y}{} P(c_i = k | z, \psi)
  \label{eq:likelihood}
\end{equation}
\noindent where $\norm{f_k(z) - y}{}$ is the error in the current
prediction.
%
The standard EM approach evaluates the expectation of~\eqref{eq:likelihood}
over many samples of $z$ and finds the parameters $(\psi, \theta)$ that
