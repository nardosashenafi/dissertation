...
systems to design a stable closed-loop system.~\todo{cite}
%
...
%
From Lyapunov stability theory~\todo{Lyapunov theory does not talk about
passivity at all!}, the system~\eqref{eq:hamiltonian_dynamics} is
passive and therefore the origin $x = (0, 0)$ is stable because~\todo{this is
not quite true... $\dot{H}$ is only negative semidefinite.}
\begin{equation*}
  \begin{gathered}
...
%
The dynamics of the resulting closed-loop system is~\todo{3.2 was also $f$}
\begin{align}
  \begin{split}  
    f(x, u) &= \bmat{\nabla_pH_d \\ -\nabla_qH_d}\, \\
  \end{split}
...
asymptotically stable system.~\todo[inline]{how is $u_{di}$ an asymptotically stable
system? rephrase.} The resulting controller is
\begin{align}
  \begin{split} 
    u &= u_{es}(x) + u_{di}(x), \\
    &u_{di}(x) = - K_{v} \, y
...
form of $H_d$ as~\todo{we don't contruct it, it is inherent.}
\begin{align}
  \Omega^\bot \left( \nabla_q H_d - \nabla_q H \right) = 0,
    \label{eq:pdes}
\end{align}
...
\begin{enumerate}
  ...
  \item Desired system behavior is explicitly introduced into the optimization
      via the performance objective $\ell$.~\todo{therefore we may find a
...
matrices $M_d$ and $J_2$.~\todo{again this constraint is not our invention. It
is inherent.} The resulting PDEs are given by 
\begin{equation}
...
in~\cite{neuralidapbc}~\todo{citation error} finds an approximate solution to the PDEs from the
...
\end{equation}~\todo{fix alignment of constraints in the optim.}
...
decomposition~\todo[inline]{By itself, Cholesky decomposition does not constrain anything. We train
$L$ in the Cholesky decomposition to get positive definiteness. Rephrase.} 
\begin{align*}
...
Suppose we are given a finite dataset from an unknown noise process~\todo{this
is not a noise process. This is a dynamical system.}, for which
we are trying to fit a regression model.~\todo[inline]{I didn't understand this.
There is a process which has noise in it. If it was just noise, you cannot
predict anything!}
%
...
distribution.~\todo{why are parameters samples? Aren't they fixed in the true
distribution?}
%
...
error~\todo{why is this quadratic, explain.} as
\begin{equation*}
...
in the data source, which requires some amount of variance.~\todo{why? explain.}
%
