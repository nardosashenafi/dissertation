...
\begin{figure}
    \centering
    \includegraphics[clip,width=0.6\linewidth]{./figures/bandplot2.eps}%
...
\textsc{NeuralPbc} framework and infer a controller that either leverages the
advantages of contact events and minimizes its adverse effects.
%
...
\begin{algorithm}
  \setstretch{1.2}
    ...
    \begin{algorithmic}[1]
        \algrenewcommand\algorithmicindent{0em} % No indent
        \State $\mathcal{D}_N \gets \{x0\}_{(N_{\mathcal{D}})}$  \Comment{$N_{\mathcal{D}}$ initial state samples} 
        \algrenewcommand\algorithmicindent{1.1em} % Change indent back to default
        \While{$i < $ \texttt{maximum iteration}}
        \State $J \gets 0$\Comment{Batch loss}
        \For{$(x_0) \sim \mathcal {D}_N$}
...
As outlined in Algorithm~\eqref{algo:bayes_neuralpbc}, we update the distribution
